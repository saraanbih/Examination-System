...
namespace Examination_system
{
    ...
    {
        #region Constructor
        public PracticalExam(TimeSpan duration, Subject subject) : base(duration, subject) { }
        #endregion
        ...
        public override void ShowExam(bool finished)
        {
            ...
            if (!finished)
            {
                ...
                while (!examCompleted && !IsTimeUp())
                {
                    Console.Clear();
                    Console.WriteLine($"\n{GetProgressInfo()}");
                    Console.WriteLine($"Time Remaining: {GetRemainingTime():hh\\:mm\\:ss}");
        ...
        #region User Interaction (Options & Input)
        ...
        #region Results (Show, Generaation, Save)
        ...
        {
            return $"\nPractical Exam Results - {Subject.SubjectName}\n" +
        ...
        private void SaveResultsToFile(string resultText)
        {
            ...
            string fileName = $"PracticalExam_{Subject.SubjectName}_{DateTime.Now:yyyyMMdd_HHmmss}.txt";
